...
import { listFilesInObsFolder, getHiddenAllowListRoots, listFilesByAdapterPaths } from "./obsFolderLister";
...
export class FakeFsLocal extends FakeFs {
  ...
  async walk(): Promise<Entity[]> {
    ...
    // [NEW] Extra scan for hidden allowlist paths
    // Obsidian doesn't load hidden folders, but we need to scan them if they're in the allowlist
    ...
    if (hiddenAllowRoots.length > 0) {
      ...
      const extraFiles = await listFilesByAdapterPaths(hiddenAllowRoots, this.vault);
